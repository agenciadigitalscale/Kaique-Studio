...
class SearchSourceTests(unittest.TestCase):
    def test_sources_by_kind(self):
        ...
        # LUTs ganhou busca (baixar look pronto); Presets seguem internos (sem busca)
        self.assertIn('FreshLUTs (grátis)', library.sources_for('LUTs'))
...
class ImageImportTests(unittest.TestCase):
    def test_import_image_kinds(self):
        self.assertIn('Ícones', library.KINDS)
        self.assertIn('Imagens', library.KINDS)
        self.assertIn('.png', library.EXTENSIONS['Imagens'])
        with tempfile.TemporaryDirectory() as d:
            base = _bundled(d)
            cat = library.Catalog(Path(d) / 'lib', base)
            img = Path(d) / 'logo.png'; img.write_bytes(bytes.fromhex('89504e470d0a'))
            added = cat.add([str(img)], 'Imagens', 'Interface')
            self.assertEqual(added[0]['kind'], 'Imagens')
            self.assertTrue(Path(added[0]['path']).is_file())
        # áudio no tipo imagem é recusado
        with tempfile.TemporaryDirectory() as d:
            base = _bundled(d); cat = library.Catalog(Path(d) / 'lib', base)
            wav = Path(d) / 'x.wav'; wav.write_bytes(b'RIFF')
            with self.assertRaises(ValueError):
                cat.add([str(wav)], 'Imagens', 'Interface')
